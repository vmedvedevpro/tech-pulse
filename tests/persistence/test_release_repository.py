...
from tests.persistence.fakes import FakeReleaseRepository
...
class TestFilterUnseen:
    ...
    async def test_returns_empty_when_all_seen(self, repo):
        ...
        assert await repo.filter_unseen(user_id=1, release_ids=[_ID_A]) == []

    async def test_returns_only_unseen_ids(self, repo):
        await repo.mark_many_seen(user_id=1, release_ids=[_ID_A])

        assert await repo.filter_unseen(user_id=1, release_ids=[_ID_A, _ID_B]) == [_ID_B]

    async def test_returns_empty_list_for_empty_input(self, repo):
        assert await repo.filter_unseen(user_id=1, release_ids=[]) == []

    async def test_seen_is_isolated_per_user(self, repo):
        await repo.mark_many_seen(user_id=1, release_ids=[_ID_A])

        assert await repo.filter_unseen(user_id=2, release_ids=[_ID_A]) == [_ID_A]
...
class TestMarkManySeen:
    ...
    async def test_marks_multiple_ids_at_once(self, repo):
        ...
        assert await repo.filter_unseen(user_id=1, release_ids=[_ID_A, _ID_B]) == []
